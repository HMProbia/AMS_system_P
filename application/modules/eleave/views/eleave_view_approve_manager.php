...
$query2 = $this->db->query($sql2)->row();
    ...
    $eleave_manager_approve=@$query2->ELEAVE_MANAGER_APPROVE;
                    ...
                    <form  method="post" enctype="multipart/form-data" >
                        ...
                        <BR>
                            ...
                            <div class="col-sm-1">
                                ...
                                <?php if($eleave_manager_approve == "1"){?>
                                    <?php if($eleave_manager_approve == "1"){ 
                                      $eleave_manager_approve = "อนุมัติแล้ว";
                                      
                                      }else if ($eleave_manager_approve == "0"){
                                        $eleave_manager_approve = "ไม่อนุมัติ";
                                        
                                      }?>

                                      <div class="form-group">
                                        <label>สถานะอนุมัติ</label>
                                        <input type="text" class="form-control"value="<?php echo $eleave_manager_approve; ?>"readonly>
                                        
                                      </div>

                                  <?php } else if($eleave_manager_approve == "0"){?>
                                  <div class="form-group">
                        ...
                        <?php if($eleave_manager_approve == "0"){ ?>
...
function check() {
  ...
  if (result.isConfirmed) {
    var pmeters = 'form_eleave_status_type=' + $("#form_eleave_status_type").val()+  '&form_eleave_id=' + $("#form_eleave_id").val()+ '&form_eleave_emp=' + $("#form_eleave_emp").val();
		  	pmeters = pmeters.replace("undefined", "");
    ///
    $.ajax({
      type: "POST",
				url: "<?= site_url(); ?>/eleave/eleave_edit_data_status_manager/",
				data: pmeters,
				success: function(resPonse){
          Swal.fire(
          'อนุมัติสำเร็จ!',
          'คุณได้ทำการอนุมัติการลาสำเร็จแล้ว !',
          'success',
          location.reload(),
         )
